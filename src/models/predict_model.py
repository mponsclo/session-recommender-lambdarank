...
def build_covisitation_matrix(con, top_k=50):
    ...
    for viewed, group in df.groupby("viewed_product"):
        covisit_dict[int(viewed)] = list(zip(group["carted_product"].astype(int), group["co_sessions"].astype(int)))
...
def load_product_catalog(con):
    ...
    for _, row in df.iterrows():
        product_info[int(row["product_id"])] = {
            ...
            "family_avg_cart_rate": float(row["family_avg_cart_rate"])
            if pd.notna(row["family_avg_cart_rate"])
            else 0.0,
            "family_popularity_rank": int(row["family_popularity_rank"])
            if pd.notna(row["family_popularity_rank"])
            else 999,
    ...
    log.info(
        f"Product catalog: {len(product_info)} products, {len(family_products)} families, {len(section_products)} sections"
    )
...
def generate_candidates(
    ...
    if viewed_with_emb:
        ...
        for idx in top_indices:
            cpid = idx_to_pid[idx]
            candidates[cpid]["cv_sim_score"] = max(candidates[cpid]["cv_sim_score"], float(sims[idx]))
...
def compute_features(
    candidates, products_viewed, product_info, session_row, cv_embeddings, cv_pid_to_idx, user_history_products
):
    ...
    max_cart_adds = max((product_info.get(pid, {}).get("total_cart_additions", 0) for pid in candidates), default=1.0)
    if max_cart_adds == 0:
        max_cart_adds = 1.0
...
# ---------------------------------------------------------------------------
# Step 6: Predict and diversify (training lives in train_model.py)
# ---------------------------------------------------------------------------
...
def main():
    ...
    # Step 6: Train LightGBM reranker (deferred import avoids the
    # train_model <-> predict_model cycle — see build_training_data)
    from train_model import build_training_data, train_ranker
    ...
    for idx, row in test_sessions.iterrows():
        ...
        session_info = {
            ...
            "discount_view_ratio": float(row.get("discount_view_ratio", 0.0))
            if pd.notna(row.get("discount_view_ratio"))
            else 0.0,
            "session_interaction_count": int(row.get("session_interaction_count", 1))
            if pd.notna(row.get("session_interaction_count"))
            else 1,
        }
        features, candidate_pids = compute_features(
            candidates, products_viewed, product_info, session_info, cv_embeddings, cv_pid_to_idx, user_hist
        )
