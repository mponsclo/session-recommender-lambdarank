...
The first stage of the two-stage pipeline. For each test session, produce ~50–150 candidate products from 8 diverse signals. Recall is the objective here — the reranker ([docs/3](3-ranking-lambdarank.md)) handles precision. All logic lives in [src/models/predict_model.py](../src/models/predict_model.py), function `generate_candidates()` at line 275.

## The 8 signals

| # | Signal | Type | Built by | Candidates / source |
|---|--------|------|----------|---------------------|
| 1 | View-to-cart co-visitation | Collaborative | `build_covisitation_matrix()` ([predict_model.py:36](../src/models/predict_model.py#L36)) | top 50 per viewed product |
| 2 | Item2Vec (Word2Vec on sessions) | Sequential | `train_item2vec()` ([predict_model.py:122](../src/models/predict_model.py#L122)) | top 15 neighbors per viewed product |
| 3 | Family top products | Metadata | inline ([predict_model.py:325-343](../src/models/predict_model.py#L325-L343)) | 15 per session family |
| 4 | CV embedding similarity | Content | `load_cv_embeddings()` ([predict_model.py:239](../src/models/predict_model.py#L239)) | top 30 by cosine similarity |
| 5 | Global popularity fallback | Popularity | inline ([predict_model.py:358-361](../src/models/predict_model.py#L358-L361)) | top 30 (max 3/family) |
| 6 | User history (returning users) | Personalized | `load_user_history()` ([predict_model.py:256](../src/models/predict_model.py#L256)) | all prior-carted products |
| 7 | Cart-to-cart co-visitation | Collaborative | `build_cart2cart_matrix()` ([predict_model.py:82](../src/models/predict_model.py#L82)) | top 30 per carted product |
| 8 | Section top products | Metadata | inline ([predict_model.py:377-387](../src/models/predict_model.py#L377-L387)) | 10 per session section |
...
- [`generate_candidates()`](../src/models/predict_model.py#L275) — per-session candidate pool
- [`main()`](../src/models/predict_model.py#L567) — pipeline orchestration
- Session loop at [predict_model.py:629-641](../src/models/predict_model.py#L629-L641)
